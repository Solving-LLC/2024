...
export function mergeTimezones(cities) {
  ...
  const left = [
    ...new Set(cities.map((city) => city.timeLeft?.totalMilliseconds)),
  ];
  return left.map((timeLeft) => {
    const filtered_cities = cities.filter(
      (city) =>
        city?.timeLeft?.totalMilliseconds < timeLeft + 1000 &&
        city?.timeLeft?.totalMilliseconds > timeLeft - 1000
    );
    const merged = {
      ...filtered_cities[0],
      ...
      timezone: [...new Set(filtered_cities.map((city) => city.timezone))].join(
        ", "
      ),
    };
    return merged;
...
export function formatTimeLeftString(difference) {
  ...
  if (seconds >= 0) {
    // Even if seconds are 0, we want to show them.
